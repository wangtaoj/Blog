...
```java
/**
 * 入口方法，刷新环境以及bean，会返回变更的key列表
 * 注意：
 * 1. 是整个配置中心涉及的PropertySource，而不是其中变化的key
 * 2. 所有被@RefreshScope标注的Bean都会被刷新
 */
public synchronized Set<String> refresh() {
    // 刷新环境且发布EnvironmentChangeEvent事件，触发被@ConfigurationProperties标注的bean重新绑定属性
...
#### 触发ContextRefresher.refresh执行的事件或者入口

Spring Cloud内部对于触发`ContextRefresher`类中的refresh方法有两个调用点

* RefreshEndpoint，actuator的一个refresh端点，通过访问/actuator/refresh端点手动触发，需要引入spring-boot-starter-actuator依赖，并且将该端点暴露出去。
* RefreshEventListener，监听RefreshEvent事件，一般给配置中心使用，配置中心监听到自己的配置发生了变化，只要发布一个RefreshEvent事件即可，这样RefreshEventListener便会执行ContextRefresher.refresh方法，完成环境属性更新，以及bean的刷新。列如nacos配置中心，有一个NacosContextRefresher类(注意与Spring Cloud中的ContextRefresher没有关联关系)，它监听到nacos配置中心的配置文件发生变化时，就会发布一个RefreshEvent事件，通知Spring重建环境以及刷新bean。

#### ContextRefresher.refresh调用后触发的事件

会先发布EnvironmentChangeEvent事件，然后发布RefreshScopeRefreshedEvent事件。

* EnvironmentChangeEvent，当Environment中的属性更新完成后，会发布该事件。从该事件中可以获取到哪些key发生了变化。如果监听这个事件，监听器的执行顺序最后要晚于`ConfigurationPropertiesRebinder`，`ConfigurationPropertiesRebinder`会触发所有被@ConfigurationProperties标注的bean中的属性重新绑定，这样可以安全使用这些bean，可以拿到更新后的属性值。
* RefreshScopeRefreshedEvent，当所有被@RefreshScope标注的bean刷新完成后，会发布该事件。

因此如果想要配置发生变更后做一些其它逻辑时，最佳的方式是监听RefreshScopeRefreshedEvent事件，比如动态线程池，调整线程池的核心线程数量以及最大线程数量等。不过需要判断下相关配置是否发生变化，因为所有的配置发生变化都会触发这些事件执行。
